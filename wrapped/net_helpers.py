...
def RGBtoGray(image):
    ...
    if (image.ndim == 2):
        ...
        image = image.astype(np.float32).sum(axis=2)
        image /= 3.0
        return image
...
def drawGraphTriangulation(h, G, triangles, image_name, dest, distance_map,
    ...
    colors = {"junction": ["orange", 3],
              "normal": ["purple", 1], "end": ["red", 2]}
...
def _drawGraph(G, verbose, n_size):
    ...
    edgelist = [(e[0], e[1])
                for e in G.edges(data=True) if e[2]['weight'] < 1000]
    ...
    if verbose:
        print("\t from _drawGraph: drawing took %1.2f sec" %
              (time.clock()-start))
