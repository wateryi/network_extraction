...
class SegImage:
    ...
    def imread(self):
        self.imgraw = cv2.imread(self.path)

    def togray(self):
        self.imggray = cv2.RGBtoGray(self.imgraw)

    def blur(self):
        blur_kernel = (cv2.gaussian_blur, cv2.gaussian_blur)
        self.imggray = cv2.blur(self.imggray, blur_kernel)
    
    def seg(self, blocksize, threshBackgrd):
        self.maskBackgrd = np.where(self.gray < threshBackgrd, 0, 1)
        image = cv2.adaptiveThreshold(self.gray, 255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                 cv2.THRESH_BINARY,blocksize,2)
        self.imgbinary = np.where(self.maskBackgrd == 0, 0, image)

    def denoise(self, minimum_feature_size, smoothing=True):
        image = self.binnary
...
class VeinNet:
    ...
    def mesh(self):
        ...
        """ 
        flattened_contours = np.asarray(self.contours)                            #add a bit of noise to increase stability of triangulation algorithm
    ...
    def classifyTriangle(self):
        ...
        self.triangles = list(np.delete(np.asarray(triangles), isolated_indices))           #remove isolated triangles from the list of triangles
    ...
    def removeRedundantNode(self, node_size=4, redundancy=0, order=5):
        ...
        if redundancy == 2: 
            nh.drawAndSafe(G,self.imageName, self.dest, parameters,self.verbose,
                ...
                self.graph_format, self.node_size)                                                   #draw and safe graph with redundant nodes
        if self.isdebug:
            nh.drawGraphTriangulation(self.height,G,triangles,self.imageName, self.dest,
        ...
        if redundancy == 1:                                                            #draw and safe graph with half redundant nodes
            ...
            nh.drawAndSafe(G,self.imageName, self.dest, parameters,self.verbose,
                self.plot, self.figure_format, self.dpi,
                self.graph_format, self.node_size)  
        
        if redundancy == 0:   
            G = nh.removeRedundantNodes(G,self.verbose,0)                                   #draw and safe graph without redundant nodes
            nh.drawAndSafe(G,self.imageName, self.dest, parameters,self.verbose,
                self.plot, self.figure_format, self.dpi,
                self.graph_format, self.node_size)  									
